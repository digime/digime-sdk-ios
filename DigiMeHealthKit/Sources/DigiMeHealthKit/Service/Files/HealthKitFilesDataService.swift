...
public class HealthKitFilesDataService: HealthKitFilesDataServiceProtocol {
	...
	public func queryData(from startDate: Date, to endDate: Date, downloadHandler: ((Result<File, SDKError>) -> Void)?, completion: ((Result<[FileListItem], SDKError>) -> Void)?) {
    ...
    private func processAndFinish() {
        ...
        Logger.mixpanel("device-data-source-read-success", metadata: HealthKitAccountDataProvider().metadata)
    }
}
