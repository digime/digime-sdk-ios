...
class ElectrocardiogramRetriever {
    func makeElectrocardiogramQuery(healthStore: HKHealthStore, predicate: NSPredicate?, sortDescriptors: [NSSortDescriptor], limit: Int, withVoltageMeasurements: Bool, resultsHandler: @escaping ElectrocardiogramResultsHandler) throws -> HKSampleQuery {
        let electrocardiogramType = ElectrocardiogramType.electrocardiogramType
        
		guard
            let type = electrocardiogramType.original as? HKElectrocardiogramType else {
            throw SDKError.invalidType(message: "\(electrocardiogramType) can not be represented as HKElectrocardiogramType")
        }
		
        let query = HKSampleQuery(sampleType: type, predicate: predicate, limit: limit, sortDescriptors: sortDescriptors) { (query, data, error) in
            guard
                error == nil,
                let results = data as? [HKElectrocardiogram] else {
                ...
                for ecgSample in results {
                    ...
                    let voltageQuery = HKElectrocardiogramQuery(ecgSample) { (query, result) in
                        ...
                        case .error(let error):
                            ...
                            group.leave()
                        @unknown default:
							ecgsError = SDKError.notAvailable(message: "Unknown case of Electrocardiogram.VoltageMeasurement result")
                            group.leave()
                        }
                    }
