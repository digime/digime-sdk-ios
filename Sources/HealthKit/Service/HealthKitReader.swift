...
typealias ActivitySummaryUpdateHandler = (HKActivitySummaryQuery, [HKActivitySummary]?, Error?) -> Void
typealias HKStatisticsCollectionHandler = (HKStatisticsCollection?, Error?) -> Void
typealias AnchoredObjectQueryHandler = (HKAnchoredObjectQuery, [HKSample]?, [HKDeletedObject]?, HKQueryAnchor?, Error?) -> Void
typealias StatisticsCollectionHandler = (HKStatisticsCollection?, Error?) -> Void
...
public class HealthKitReader {
	...
	public func characteristics() -> Characteristic {
		...
		if #available(iOS 14.0, *) {
			...
			return Characteristic(biologicalSex: biologicalSex,
								  birthday: birthday,
								  bloodType: bloodType,
     ...
     */
	public func quantityQuery(type: QuantityType,
							  unit: String,
							  ...
							  resultsHandler: @escaping QuantityResultsHandler) throws -> SampleQuery {
		...
		let query = HKSampleQuery(sampleType: quantityType, predicate: predicate, limit: limit, sortDescriptors: sortDescriptors) { (query, data, error) in
            guard
                error == nil,
                let results = data else {
    ...
    public func categoryQuery(
        ...
        guard let sampleType = type.original as? HKCategoryType else {
			throw SDKError.invalidType(message: "\(type) can not be represented as HKCategoryType")
        }
    ...
    public func workoutQuery(
        ...
        guard let type = workoutType.original as? HKWorkoutType else {
            throw SDKError.invalidType(
				message: "\(workoutType) can not be represented as HKWorkoutType"
            )
        }
        let query = HKSampleQuery(
            sampleType: type,
    ...
    public func correlationQuery(
        ...
        guard let correlationType = type.original as? HKCorrelationType else {
            throw SDKError.invalidType(
				message: "\(type) can not be represented as HKWorkoutType"
            )
        }
        let query = HKSampleQuery(
    ...
    public func sampleQuery(
        ...
        guard let sampleType = type.original as? HKSampleType else {
            throw SDKError.invalidType(
				message: "\(type) can not be represented as HKSampleType"
            )
        }
        let query = HKSampleQuery(
            sampleType: sampleType,
	...
	/**
	 ...
	 - Parameter mergeResultForSameType: **Bool** set true to merge all data into a single result updates. False by default.
    ...
    /**
     ...
     - Throws: SDKError.invalidType
     */
	public func heartbeatSeriesQuery(predicate: NSPredicate? = .allSamples,
									 sortDescriptors: [NSSortDescriptor] = [
										NSSortDescriptor(
											key: HKSampleSortIdentifierStartDate,
											ascending: false
									 ...
									 limit: Int = HKObjectQueryNoLimit,
									 resultsHandler: @escaping HeartbeatSeriesResultsDataHandler) throws -> SampleQuery {
		return try SeriesSampleRetriever().makeHeartbeatSeriesQuery(healthStore: healthStore,
																	predicate: predicate,
																	sortDescriptors: sortDescriptors,
																	limit: limit,
																	resultsHandler: resultsHandler)
    ...
    /**
     ...
     - Throws: SDKError.invalidType
	 */
	public func workoutRouteQuery(predicate: NSPredicate? = .allSamples,
								  sortDescriptors: [NSSortDescriptor] = [
									NSSortDescriptor(
										key: HKSampleSortIdentifierStartDate,
										ascending: false
								  ...
								  limit: Int = HKObjectQueryNoLimit,
								  resultsHandler: @escaping WorkoutRouteResultsDataHandler) throws -> SampleQuery {
		return try SeriesSampleRetriever().makeWorkoutRouteQuery(healthStore: healthStore,
																 predicate: predicate,
																 sortDescriptors: sortDescriptors,
																 limit: limit,
																 resultsHandler: resultsHandler)
     ...
     */
    public func queryActivitySummary(predicate: NSPredicate? = nil, monitorUpdates: Bool = false, completionHandler: @escaping ActivitySummaryCompletionHandler) -> ActivitySummaryQuery {
    ...
    /**
     ...
     - Throws: SDKError.invalidType
     */
	public func anchoredObjectQuery(type: SampleType,
									predicate: NSPredicate? = .allSamples,
									...
									limit: Int = HKObjectQueryNoLimit,
									monitorUpdates: Bool = false,
									completionHandler: @escaping AnchoredResultsHandler) throws -> AnchoredObjectQuery {
    ...
    /**
     ...
     - Throws: SDKError.invalidType
     */
    public func sourceQuery(type: SampleType, predicate: NSPredicate? = .allSamples, completionHandler: @escaping SourceCompletionHandler) throws -> SourceQuery {
        guard let sampleType = type.original as? HKSampleType else {
            throw SDKError.invalidType(
				message: "\(type) can not be represented as HKSampleType"
            )
        ...
        let query = HKSourceQuery(sampleType: sampleType, samplePredicate: predicate) { (_, data, error) in
            guard
                error == nil,
                let result = data else {
    ...
    /**
     ...
     - Throws: SDKError.invalidType
     */
    public func correlationQuery(type: CorrelationType, predicate: NSPredicate? = .allSamples, typePredicates: [String: NSPredicate]? = nil, completionHandler: @escaping CorrelationCompletionHandler) throws -> CorrelationQuery {
        guard let correlationType = type.original as? HKCorrelationType else {
            throw SDKError.invalidType(
				message: "\(type) can not be represented as HKCorrelationType"
            )
        }
		
        let query = HKCorrelationQuery(type: correlationType, predicate: predicate, samplePredicates: typePredicates?.sampleTypePredicates) { (_, data, error) in
            guard
                error == nil,
                let result = data else {
	...
	private func mergeData(data: [Statistics]) throws -> Statistics {
		...
		for obj in data {
			...
			else if let obj2 = result {
				result = try obj.merge(obj1: obj, obj2: obj2)
			}
		}
		
		guard let result = result else {
			throw SDKError.invalidType(message: "Error merge Statistics data")
		}
