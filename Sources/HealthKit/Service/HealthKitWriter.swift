...
public class HealthKitWriter {
    ...
    /**
     ...
     - Throws: `SDKError.notAvailable` `SDKError.invalidType`
    ...
    public func isAuthorizedToWrite(type: ObjectType) throws -> Bool {
        guard let objectType = type.original else {
			throw SDKError.invalidType(message: "Invalid type: \(type)")
        }
        ...
        case .sharingAuthorized:
            return true
        @unknown default:
			throw SDKError.notAvailable(message: "Invalid status")
        }
    }
    ...
    /**
     ...
     - Parameter completion: block notifies about operation status
     */
    public func delete(sample: Sample, completion: @escaping StatusCompletionBlock) {
        do {
     ...
     */
    public func deleteObjects(of objectType: ObjectType, predicate: NSPredicate, completion: @escaping DeletionCompletionBlock) {
        guard let type = objectType.original else {
            completion(false, -1, SDKError.invalidType(message: "Object type was invalid: \(objectType)"))
            return
        }
    ...
    /**
     ...
     - Parameter completion: block notifies about operation status
     */
    public func save(sample: Sample, completion: @escaping StatusCompletionBlock) {
        do {
            if let quantity = sample as? Quantity {
	...
	/**
	 Saves the HK object array
	 - Parameter data: **Sample** sample
	 - Parameter completion: block notifies about operation status
	 */
	public func saveData(_ data: [HKObject], completion: @escaping (_ success: Bool, _ error: Error?) -> Void) {
