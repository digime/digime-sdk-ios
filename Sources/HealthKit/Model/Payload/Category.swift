...
public struct Category: PayloadIdentifiable, Sample {
	...
	public func copyWith(identifier: String? = nil,
						 ...
						 harmonized: Harmonized? = nil) -> Category {
		return Category(identifier: identifier ?? self.identifier,
	...
	func asOriginal() throws -> HKCategorySample {
		guard let type = identifier.objectType?.original as? HKCategoryType else {
			throw SDKError.invalidType(
				message: "Category type identifier: \(identifier) could not be formatted"
			)
		}
...
extension Category.Harmonized: Payload {
	public static func make(from dictionary: [String: Any]) throws -> Category.Harmonized {
		...
		return Category.Harmonized(value: value,
