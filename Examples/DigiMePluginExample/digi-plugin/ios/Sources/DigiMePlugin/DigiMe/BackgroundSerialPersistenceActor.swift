...
///  // so you must have one instance of the Actor for one container for it to work correctly.
///  
...
public actor BackgroundSerialPersistenceActor: ModelActor {
    ...
    public func count<T: PersistentModel>(for type: T.Type, predicate: Predicate<T>? = nil) throws -> Int {
        return try fetchCount(predicate: predicate)
    }
    ...
    public func fetchData<T: PersistentModel>(
        ...
        fetchDescriptor.fetchLimit = fetchLimit ?? 0
        fetchDescriptor.fetchOffset = fetchOffset ?? 0
        let list: [T] = try context.fetch(fetchDescriptor)
        return list
    }
}
