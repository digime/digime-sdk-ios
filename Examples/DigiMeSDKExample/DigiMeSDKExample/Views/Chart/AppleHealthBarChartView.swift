			...
			if !viewModel.result.isEmpty {
				VStack(alignment: .leading) {
					...
					case .allTime:
						...
						MonthlyActivityChart(data: $viewModel.result, dataType: .distanceWalkingRunning, barColor: .blue)
							.frame(height: 240)
						
						getBarTitleView(data: viewModel.result, dataType: .activeEnergyBurned, title: "Active Energy Burned", unit: "cal")
						MonthlyActivityChart(data: $viewModel.result, dataType: .activeEnergyBurned, barColor: .blue)
							.frame(height: 240)
					}
				}
