...
public class DMEJWTManager: NSObject {
    ...
    @objc public func createAndSignPreAuthToken(_ appId: String, contractId: String, privateKeyBase64: String, publicKeyBase64: String?) -> String? {
        ...
        guard let privateKey = convertKeyString(privateKeyBase64) else {
        ...
        // validation
        guard
            let publicKeyBase64 = publicKeyBase64,
    ...
    // we are creating our own `request` JWT to be able to get preauthentication token wrapped in server side created JWT
    @objc public func createAndSignAuthToken(_ authCode: String, appId: String, contractId: String, privateKeyBase64: String, publicKeyBase64: String?) -> String? {
    ...
    @objc public func validateAndDecodeOngoingAccessAuthAndRefreshTokensFromJWT(_ authToken: String) -> DMEOAuthObject? {
        guard
    ...
    @objc public func createDataTriggerToken(_ accessToken: String, appId: String, contractId: String, sessionKey: String, privateKeyBase64: String, publicKeyBase64: String?) -> String? {
        let claims = PayloadDataTriggerJWT()
        claims.accessToken = accessToken
        claims.clientId = "\(appId)_\(contractId)"
        claims.nonce = (DMECryptoUtilities.getRandomBytes(withLength: 16) as NSData).hexString()
        // NB! this redirect schema must exist in the CA contract definition (Cadmium/Neon tool), otherwise this request will fail!
        claims.redirectUrl = "digime-ca-\(appId)"
        ...
        let verifier = JWTVerifier.ps512(publicKey: publicKey)
        let isVerified = JWT<PayloadDataTriggerJWT>.verify(signedToken, using: verifier)

        return isVerified ? signedToken : nil
    }
    
    @objc public func createRefreshToken(_ refreshToken: String, appId: String, contractId: String, privateKeyBase64: String, publicKeyBase64: String?) -> String? {
        ...
        let verifier = JWTVerifier.ps512(publicKey: publicKey)
        let isVerified = JWT<PayloadRefreshOAuthJWT>.verify(signedToken, using: verifier)
